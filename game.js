...
const EGI_SHOP = [
    ...
    { id: "blue", name: "エギ王 Live (ブルー)", price: 1500, speed: 0.13, color: "#00bfff", headColor: "#00008b", sinkerColor: "#4682b4", featherColor: "#ffffff", pattern: "stripes", desc: "澄み潮用カラー" },
    ...
    { id: "green", name: "エギ王 K (軍艦グリーン)", price: 5000, speed: 0.16, color: "#2f4f4f", headColor: "#006400", sinkerColor: "#8b0000", featherColor: "#da7706", pattern: "spots", effect: "shimmer", desc: "伝説の人気カラー" },
    { id: "cocktail", name: "エギ王 K (カクテルオレンジ)", price: 5000, speed: 0.16, color: "#ff8c00", headColor: "#ff4500", sinkerColor: "#ffd700", featherColor: "#da7706", pattern: "spots", desc: "濁り潮に強い" },
    ...
    { id: "search", name: "エギ王 Search (ラトル)", price: 12000, speed: 0.20, color: "#ffd700", headColor: "#ff0000", sinkerColor: "#ffd700", featherColor: "#ffffff", pattern: "arrows", effect: "vibrate", desc: "ハイアピール型" },
    ...
    { id: "majyo", name: "魔女クロス・パープル", price: 35000, speed: 0.30, color: "#483d8b", headColor: "#9932cc", sinkerColor: "#2f4f4f", featherColor: "#00ffff", pattern: "dartmax", effect: "majyo", desc: "怪しく光る深場用" },
    { id: "dartmax_purple", name: "ダートマックスTrパープル", price: 50000, speed: 0.35, color: "#9932cc", headColor: "#ff00ff", sinkerColor: "#4b0082", featherColor: "#00ffff", pattern: "rainbow", effect: "rainbow", desc: "伝説の餌木", secret: true },
    { id: "ultra_gold", name: "黄金のレッドモンスター極", price: 100000, speed: 0.45, color: "#ffd700", headColor: "#ffffff", sinkerColor: "#ffd700", featherColor: "#ffffff", pattern: "glow", effect: "aura", desc: "全てを凌駕する最高級エギ", secret: true }
];
...
// --- ティップラン・リアリズム用変数 ---
let worldCurrentSpeed = 0.5; // 潮流 (kt)
let lineAngle = 0; // ライン角度 (度)
let selectedSinker = 0; // 装着中のシンカー (g)
let isSnagged = false; // 根掛かりフラグ
let snagClearCount = 0; // 根掛かり解除のための連打数
let hitType = 'pull'; // 'pull' (入り・引き), 'back' (戻り)
let egiOffsetX = 0; // 潮流によるエギのXオフセット
let seabedSnagTimer = 0; // 着底放置タイマー

// --- 高度な機能用変数 ---
let gameTime = 12; // 0-24
let timeOfDay = 'day'; // 'morning', 'day', 'evening', 'night'
let inkAlpha = 0; // スミの透明度
let landingProgress = 0; // タモ入れの進行度
let landingTargetScale = 0; // タモ入れのターゲット円サイズ
let rivals = [
    { name: "キャプテン", score: 0, catchRate: 0.03 },
    { name: "クロミ", score: 0, catchRate: 0.05 },
    { name: "バク", score: 0, catchRate: 0.02 }
];
let lastRivalUpdateTime = 0;
...
// resize関数とイベントリスナーを削除し、draw内で毎フレーム監視する方式に変更
// これにより、モバイルのURLバーの出し入れ等でサイズが変わっても即座に対応可能
...
function drawEgi(x, y, egi, angle) {
    ...
    // 特殊エフェクト: オーラ (aura, majyo)
    if (egi.effect === 'aura' || egi.effect === 'majyo' || egi.effect === 'rainbow') {
        ctx.save();
        ctx.shadowBlur = 15 + Math.sin(t * 2) * 10;
        if (egi.effect === 'aura') ctx.shadowColor = "#ffd700";
        else if (egi.effect === 'majyo') ctx.shadowColor = "#9932cc";
        else if (egi.effect === 'rainbow') ctx.shadowColor = `hsl(${(Date.now() / 10) % 360}, 100%, 50%)`;

        // オーラのゆらぎを描画（本体より一回り大きく）
        ctx.globalAlpha = 0.3;
        ctx.fillStyle = ctx.shadowColor;
        ctx.beginPath();
        ctx.ellipse(5, -2, 45, 18, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    }
    ...
    let drawColor = color;
    if (egi.effect === 'rainbow') {
        drawColor = `hsl(${(Date.now() / 20) % 360}, 100%, 70%)`;
    }

    bodyGrad.addColorStop(0, drawColor);
    bodyGrad.addColorStop(0.5, (pattern === 'glow' || egi.effect === 'aura') ? "#fffce0" : "#fff");
    bodyGrad.addColorStop(1, drawColor);
    ctx.fillStyle = bodyGrad;
    ctx.fill();

    // 特殊エフェクト: ラメ (shimmer)
    if (egi.effect === 'shimmer') {
        ctx.save();
        ctx.clip(); // ボディ内に制限
        ctx.fillStyle = "#fff";
        for (let i = 0; i < 15; i++) {
            const px = Math.sin(t * 5 + i) * 40;
            const py = Math.cos(t * 3 + i * 2) * 15;
            const size = Math.abs(Math.sin(t * 10 + i)) * 2;
            ctx.globalAlpha = Math.max(0, Math.sin(t * 5 + i));
            ctx.beginPath();
...
function drawBoat(sway, scale = 1.0) {
    ctx.save();
...
function drawRod(tipX, tipY, sway, scale = 1.0) {
    ctx.save();
    ctx.translate(0, sway * 15);
    ...
    // モバイル時はグリップ位置も少し調整（船のスケールに合わせる）
    if (scale < 1.0) {
        // 船の基準点(20) + 140 * scale (元の160は20+140)
        startX = 20 + 140 * scale;
        // Yも同様に調整したいが、複雑になるので簡易的に
    }

    // グリップ描画
    ctx.save();
    ctx.translate(startX, startY);
    ctx.scale(scale, scale); // グリップも縮小
    ctx.beginPath();
    ...
    // 2. ブランクス
    // グリップ先端位置 (スケール考慮)
    let bX = startX + 88 * scale, bY = startY + 7 * scale;
    let dx = tipX - bX, dy = tipY - bY;

    // ティップラン調（ファーストテーパー）の実現
    // バット側(cp1)は硬く直線的に、ティップ側(cp2)で急激に曲がるように制御点を調整
    let cp1x = bX + dx * 0.6; // 60%付近まで張りを保つ
    let cp1y = bY + dy * 0.1; // 縦方向の曲がりは抑える

    let cp2x = bX + dx * 0.9; // 90%付近で
    let cp2y = bY + dy * 0.5; // グッと落とす
    ...
    ctx.strokeStyle = blankGrad; ctx.lineWidth = (isLimited ? 5 : 4) * scale;
    ctx.bezierCurveTo(cp1x, cp1y, cp2x, cp2y, tipX, tipY);
    ctx.stroke();

    // 3. ガイド
    ctx.strokeStyle = isLimited ? "#fbbf24" : "#71717a"; ctx.lineWidth = 1 * scale;
    const guideCount = isLimited ? 7 : 5;
    for (let i = 1; i <= guideCount; i++) {
        // ガイドの配置も曲線の変化に合わせて非線形に（先端に寄せる）
        let t = Math.pow(i / guideCount, 0.6);
        let invT = 1 - t;
        ...
        ctx.beginPath(); ctx.arc(gx, gy, 2 * scale, 0, Math.PI * 2); ctx.stroke();
        ctx.fillStyle = isLimited ? "#fbbf24" : "#333";
        ctx.fillRect(gx - 2 * scale, gy - 1 * scale, 4 * scale, 3 * scale);
    }
...
function actionFall() {
    ...
    // 潮流の変化
    worldCurrentSpeed = (0.2 + Math.random() * 1.5).toFixed(1);
    document.getElementById('current-val').innerText = worldCurrentSpeed;
    lineAngle = 0;
    egiOffsetX = 0;
    isSnagged = false;
    seabedSnagTimer = 0;
        ...
        jerkFallbackTimer = setTimeout(() => {
            if (state === 'staying' || state === 'bottom') {
                if (isSnagged) {
                    ...
                    if (snagClearCount > 3) {
                        isSnagged = false;
                        msg.innerText = "根掛かりが外れた！";
                        msg.classList.remove('snagged');
                        snagClearCount = 0;
                        seabedSnagTimer = 0;
                    }
                } else {
...
function actionHook() {
    if (state === 'landing') {
        const diff = Math.abs((1 - landingProgress) - landingTargetScale);
        if (diff < 0.1) processLandingWin();
        else endGame("タモ入れ失敗...バラした！");
        return;
    }
    if (state === 'hit') {
        ...
        // 1/100の確率でサメが襲来（元1/10から低減）
        isSharkAttacking = Math.random() < 0.01;
        if (isSharkAttacking) {
...
function drawDepthMeter(currentD, targetD, surfaceY) {
    ...
    const meterYEnd = canvas.height - 20; // 画面高さに合わせて調整
...
function draw() {
    // 毎フレームキャンバスのサイズを確認し、変更があれば適用する（レスポンシブ対応の強化）
    // これにより、画面回転やズーム、UIの表示非表示によるサイズ変化に確実に追従する
    ...
    if (feverTimer > 0) {
        ...
        if (state === 'falling') {
            const sinkerBonus = selectedSinker * 0.005;
            currentDepth += (selectedEgi.speed + sinkerBonus) * 2; // 沈下2倍速
        }
    }
    ...
    ctx.fillRect(0, 0, canvas.width, canvas.height); // 一旦全面を空の色で塗る(canvas.heightを使用)

    // Water Area (Below Wave)
    let grd = ctx.createLinearGradient(0, surfaceY, 0, canvas.height);
    ...
    // UI Scale calculation (モバイル対応)
    const isMobileUI = canvas.width < 600;
    // 画面の1/3に収まるように大幅に縮小
    const uiScale = isMobileUI ? 0.45 : 1.0;

    // 時間経過 (1日 = 600秒 / 10分 に設定)
    gameTime = (gameTime + 0.000667) % 24;
    updateTimeOfDay();

    // ライバル更新
    if (Date.now() - lastRivalUpdateTime > 5000) {
        updateRivals();
        lastRivalUpdateTime = Date.now();
    }

    // スミの減衰
    if (inkAlpha > 0) inkAlpha = Math.max(0, inkAlpha - 0.002);
    document.getElementById('ink-overlay').style.opacity = inkAlpha;

    // 背景描画（時間帯に応じた色）
    drawBackground();

    drawBoat(boatSway, uiScale);

    // 潮流とライン角度の物理演算
    if (state === 'falling' || state === 'bottom' || state === 'staying') {
        // シンカーによる角度抑制 (重いほど角度がつきにくい)
        const sinkerEffect = 1.0 - (selectedSinker / 100);
        lineAngle += (worldCurrentSpeed * 0.05 * sinkerEffect);
        if (lineAngle > 70) lineAngle = 70; // 最大角度

        // Xオフセットの計算 (深度と角度に基づく)
        egiOffsetX = Math.tan(lineAngle * Math.PI / 180) * currentDepth * 2;
        document.getElementById('angle-val').innerText = Math.floor(lineAngle);

        // 潮流による泡の速さ調整
        bubbles.forEach(b => b.x -= worldCurrentSpeed * 2);
    }
    ...
    const seaBedY = canvas.height - 20;

    let seabedGrd = ctx.createLinearGradient(0, seaBedY, 0, canvas.height);
    seabedGrd.addColorStop(0, "#1a1105");
    seabedGrd.addColorStop(1, "#000");
    ctx.fillStyle = seabedGrd;
    ctx.fillRect(0, seaBedY, canvas.width, 20);

    seabedObjects.forEach(obj => {
        obj.x -= 0.6; if (obj.x < -60) obj.x = canvas.width + 60;

        // オブジェクトの基準Y座標も canvas.height に追従
        const objBaseY = canvas.height - 15;

        if (obj.type === 'weed') {
            ctx.strokeStyle = "#005a32"; ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(obj.x, objBaseY + 5);
            ctx.bezierCurveTo(obj.x - 10, objBaseY, obj.x, objBaseY + 5 - obj.size / 2, obj.x, objBaseY + 5 - obj.size); ctx.stroke();
        } else { ctx.fillStyle = "#3d2b1f"; ctx.fillRect(obj.x, objBaseY - 5, obj.size, 12); }
    });

    // エギの深度計算ロジックの抜本的修正
    // これまでは固定値オフセットで計算していたため、画面サイズが変わるとズレが生じていた
    // 今後は「水面(surfaceY)」から「海底(canvas.height - 20)」までの距離を動的に計算し、その割合で配置する

    // 修正: ターゲット深度が浅い場合(例:10.8m)に、maxDが15m固定になってしまい、
    // 到達しても比率が1.0にならずエギが中層で止まる問題を解消。
    // targetDepthがあればそれを分母とし、未設定(0)の時のみデフォルト値を使う。
    const maxD = (targetDepth > 0.1) ? targetDepth : 15;
    const seabedY = canvas.height - 20; // 海底のY座標（描画位置と完全に一致させる）

    // エギの描画位置Y
    // currentDepth / maxD の進行度に応じて、surfaceY から seabedY の間を移動
    let egiY = surfaceY + (currentDepth / maxD) * (seabedY - surfaceY);

    if (state === 'fighting') {
        // ファイト中も同様に計算
        egiY = surfaceY + (squidDist / maxD) * (seabedY - surfaceY);
    }

    // エギが海底に埋まらないようにキャップ（理論上はmaxDで一致するが浮動小数誤差対策）
    egiY = Math.min(egiY, seabedY - 10);

    // モバイル時は画面全体の1/3程度にロッド先端を収める

    // モバイル時は画面全体の1/3程度にロッド先端を収める
    let tipX = isMobileUI ? canvas.width * 0.35 : 450;
    let tipY = 100;
    if (state === 'hit') {
        if (hitType === 'pull') tipY += 30; // 引き込み
        else tipY -= 20; // 戻りアタリ
    }
    if (state === 'fighting') tipY += (tension / 2);

    // Draw Rod and Line
    drawRod(tipX, tipY, boatSway, uiScale);
    ...
    for (let i = approachingSquids.length - 1; i >= 0; i--) {
        ...
        if (s.fleeing) {
            ...
            // 画面外または透明になったら削除
            if (s.alpha <= 0 || s.x < -100 || s.x > canvas.width + 500 || s.y < -100 || s.y > canvas.height + 100) {
    ...
    let drawEgiX = egiX + egiOffsetX;
    ctx.lineTo(drawEgiX, egiY); ctx.stroke();

    let egiAngle = (state === 'falling') ? -Math.PI / 6 : (state === 'retrieving' ? Math.PI / 6 : -0.15);
    // ライン角度によるエギの傾き追加
    egiAngle += (lineAngle * Math.PI / 180) * 0.5;
    drawEgi(drawEgiX, egiY, selectedEgi, egiAngle);
    ...
    if (state === 'falling') {
        if (currentDepth < targetDepth) {
            const sinkerBonus = selectedSinker * 0.005;
            currentDepth += (selectedEgi.speed + sinkerBonus) * (feverTimer > 0 ? 2 : 1);
            ...
            // 着底放置による根掛かり判定
            seabedSnagTimer += 1 / 60;
            if (!isSnagged && seabedSnagTimer > 5.0 && Math.random() < 0.005) {
                isSnagged = true;
                msg.innerText = "⚠️ 根掛かり！シャクリ連打で外せ！";
                msg.classList.add('snagged');
            }
        }
    }
    ...
    if (state === 'fighting') {
        if (isSharkAttacking) {
            ...
            if (!isReeling) {
                ...
                // スミ噴射 (視界遮断) の抽選
                if ((attackerSquid.isRed || attackerSquid.isGiant) && Math.random() < 0.005) {
                    inkAlpha = 1.0;
                }
        ...
        if (state === 'fighting' && attackerSquid) {
            ...
            // 距離が0になるか、海面(egiY)が波の高さ(surfaceY)に達したらランディングフェーズへ
            if (squidDist <= 0 || egiY <= surfaceY + 20) {
                state = 'landing';
                landingProgress = 0;
                landingTargetScale = 0.3 + Math.random() * 0.4;
                ...
                msg.innerText = "タモを構えろ！タイミングを合わせて「合わせ」！";
            }
        }
    }

    if (state === 'landing' && attackerSquid) {
        landingProgress += 0.008;
        if (landingProgress > 1) {
            endGame("タモ入れ失敗...バラした！");
        } else {
            const lY = surfaceY + 20;
            const lX = canvas.width / 2;
            if (attackerSquid.isKuromi) {
                drawKuromi(lX, lY, attackerSquid.scale, 1, 0, true);
            } else {
                drawSquid(lX, lY, attackerSquid.scale, 1, 0, attackerSquid.color, true, true);
            }
            const pC = document.getElementById('landing-circle-player');
            const tC = document.getElementById('landing-circle-target');
            if (pC) pC.style.transform = `translate(-50%, -50%) scale(${1 - landingProgress})`;
            if (tC) tC.style.transform = `translate(-50%, -50%) scale(${landingTargetScale})`;
        }
    }
...
function triggerHit() {
    ...
    state = 'hit';
    hitType = Math.random() < 0.4 ? 'back' : 'pull'; // 40%で戻りアタリ
    ...
    if (attackerSquid.isKuromi) {
        ...
        switch (finalEvent) {
            ...
            default:
                let hitMsg = hitType === 'back' ? "竿先が戻った！" : "💥 アタリ！";
                msg.innerText = attackerSquid.isGiant ? "⚠️ 警告：ダイオウイカ出現！" : (attackerSquid.isRed ? "💥 巨大なアタリ！" : hitMsg);
        }
    }
    document.getElementById('btn-hook').style.display = 'block';
    document.getElementById('btn-reel').style.display = 'block';
    setBtn('btn-retrieve', false); // ヒット中は回収不可
    setBtn('btn-fall', false);     // フォール不可
    setBtn('btn-jerk', false);     // シャクリ不可
    setBtn('btn-stay', false);     // ステイ不可
    let hookTime = (feverTimer > 0) ? 2000 : 1300;
    timers.push(setTimeout(() => { if (state === 'hit') { if (isSnagged) isSnagged = false; endGame("見切られた..."); } }, hookTime));
}
...
function endGame(t) {
    ...
    approachingSquids = [];
    inkAlpha = 0;
    document.getElementById('ink-overlay').style.opacity = 0;
...
function startGame() {
    ...
    selectedSinker = parseInt(document.getElementById('select-sinker').value) || 0;
    document.getElementById('disp-sinker').innerText = selectedSinker;
...
function updateSelections() {
    ...
    // 現在の選択値を保持（または保存された設定値を使用）
    // DOMから読み取るか、変数が初期化されていればそれを使う
    const curLoc = locS.value || (selectedLocObj ? selectedLocObj.id : LOCATIONS[0].id);
    const curRod = rodS.value || currentRodId;
    const curReel = reelS.value || currentReelId;
    ...
    // 選択状態を復元（存在する場合のみ）
    // これにより「選択できない（勝手に戻る）」不具合を解消
    if (Array.from(locS.options).some(o => o.value === selectedLocObj.id)) locS.value = selectedLocObj.id;
    if (Array.from(rodS.options).some(o => o.value === currentRodId)) rodS.value = currentRodId;
    if (Array.from(reelS.options).some(o => o.value === currentReelId)) reelS.value = currentReelId;
}
...
rb.onmousedown = () => {
    if (state === 'hit') actionHook();
    isReeling = true;
};
window.onmouseup = () => isReeling = false;
rb.ontouchstart = (e) => {
    e.preventDefault();
    if (state === 'hit') actionHook();
    isReeling = true;
};
rb.ontouchend = (e) => { e.preventDefault(); isReeling = false; };

// タッチ操作の追加設定
function setupTouchControls() {
    const attachTouch = (id, func) => {
        const btn = document.getElementById(id);
        if (!btn) return;
        btn.addEventListener('touchstart', (e) => {
            e.preventDefault(); // ダブルタップズームやスクロール防止
            if (!btn.disabled) func();
        }, { passive: false });
    };

    attachTouch('btn-fall', actionFall);
    attachTouch('btn-jerk', actionJerk);
    attachTouch('btn-stay', actionStay);
    attachTouch('btn-retrieve', actionRetrieve);
    attachTouch('btn-hook', actionHook);
...
window.onload = () => {
    ...
    updateTackleDisplay();
    initAtmosphere();
    setupTouchControls(); // タッチ操作の初期化

    // キャンバスクリック/タップで合わせ・タモ入れを可能にする
    canvas.onmousedown = (e) => {
        if (state === 'landing' || state === 'hit') actionHook();
    };
    canvas.addEventListener('touchstart', (e) => {
        if (state === 'landing' || state === 'hit') {
            e.preventDefault(); // キャンバスでのスクロール等を防止
            actionHook();
        }
    }, { passive: false });
...
function processLandingWin() {
    ...
    if (attackerSquid.isKuromi) {
        ...
        kuromiCaughtCount++;
        bonusMoney = 15000;
        endGame(`😈 クロミ様を釣り上げた！世界クロミ化計画！ 🎉 (15,000G GET)`);
        ...
        if (attackerSquid.event === 'golden') {
            multiFactor = 4; // 10倍から4倍に削減
            eventMsg = "✨ 黄金のイカ獲得！4倍ボーナス！ ";
        } else if (attackerSquid.event === 'baku') {
            bonusMoney = 10000; // 50,000Gから10,000Gに削減
            eventMsg = "💜 バクのボーナス 10,000G GET! ";
    ...
    // 換金率を1.0から0.4に修正
    money += Math.floor(weight * multiFactor * 0.4) + bonusMoney;
    checkUnlocks(score + redCount - multiFactor, score + redCount);
}

function updateTimeOfDay() {
    const prevTime = timeOfDay;
    if (gameTime >= 5 && gameTime < 8) timeOfDay = 'morning';
    else if (gameTime >= 8 && gameTime < 16) timeOfDay = 'day';
    else if (gameTime >= 16 && gameTime < 19) timeOfDay = 'evening';
    else timeOfDay = 'night';

    if (prevTime !== timeOfDay) {
        const display = document.getElementById('time-display');
        const icons = { morning: "🌄 朝", day: "☀️ 昼", evening: "🌆 夕", night: "🌙 夜" };
        const colors = { morning: "#ffaa00", day: "#ffcc00", evening: "#ff4500", night: "#8888ff" };
        if (display) {
            display.innerText = icons[timeOfDay];
            display.style.color = colors[timeOfDay];
        }
    }
}
...
function updateRivals() {
    ...
    list.innerHTML = '';
    rivals.forEach(r => {
        if (Math.random() < r.catchRate) r.score++;
        const div = document.createElement('div');
        div.className = 'rival-item';
        div.innerHTML = `<span class="rival-name">${r.name}</span><span class="rival-score">${r.score}</span>`;
        if (list) list.appendChild(div);
    });
}

function drawBackground() {
    const surfaceY = 84 + boatSway * 15;
    let topColor, bottomColor, waterTop, waterBottom;
    switch (timeOfDay) {
        case 'morning':
            topColor = "#ff7e5f"; bottomColor = "#feb47b";
            waterTop = "#004e92"; waterBottom = "#000428";
            break;
        case 'evening':
            topColor = "#ef4444"; bottomColor = "#f59e0b";
            waterTop = "#1e1b4b"; waterBottom = "#0f172a";
            break;
        case 'night':
            topColor = "#0f172a"; bottomColor = "#1e1b4b";
            waterTop = "#020617"; waterBottom = "#000000";
            break;
        default: // day
            topColor = "#00d2ff"; bottomColor = "#3a7bd5";
            waterTop = "#004e92"; waterBottom = "#000428";
            break;
    }

    const skyGrad = ctx.createLinearGradient(0, 0, 0, surfaceY);
    skyGrad.addColorStop(0, topColor); skyGrad.addColorStop(1, bottomColor);
    ctx.fillStyle = skyGrad; ctx.fillRect(0, 0, canvas.width, surfaceY);

    const oceanGrad = ctx.createLinearGradient(0, surfaceY, 0, canvas.height);
    oceanGrad.addColorStop(0, waterTop); oceanGrad.addColorStop(1, waterBottom);
    ctx.fillStyle = oceanGrad; ctx.fillRect(0, surfaceY, canvas.width, canvas.height - surfaceY);
}
